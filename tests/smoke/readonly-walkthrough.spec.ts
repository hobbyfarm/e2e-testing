...
test('UI walkthrough readonly smoke test', async ({ page }) => {
  ...
  let homePage = await loginPage.fillCredentialsAndSubmit(process.env.HOBBYFARM_UI_NAME as string, process.env.HOBBYFARM_ADMIN_UI_USR as string, process.env.HOBBYFARM_ADMIN_UI_PWD as string);
  await homePage.displayAboutModal();
  await homePage.displayLogoutModal();
